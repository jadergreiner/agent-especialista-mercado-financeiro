...
Este documento descreve a política mínima para mascaramento (masking) de dados sensíveis (PII) em ambientes não produtivos e em endpoints de API que expõem informações de usuários ou clientes.

## Escopo

- Aplicável a: staging, ambientes locais com dados reais, dumps/exports e endpoints administrativos.
- Não aplicar masking em operações normais no banco de produção; nesses casos, usar RBAC e logging seguro.

## Definições

- **PII**: Personally Identifiable Information (ex.: nome, email, CPF/SSN, telefone, documentos).
- **Masking**: transformação que oculta PII (irreversível ou parcialmente reversível conforme política).

## Regras básicas (mínimo)

1. Endpoints de staging e públicos devem retornar PII mascarado por padrão.
2. Campos sensíveis iniciais (heurística): `name`, `email`, `cpf`, `ssn`, `document`, `phone`, `telefone`.
3. Exportações administrativas (CSV/JSON) só por role `admin` e auditadas.
4. Usar `scripts/mask_sqlite.py` para gerar cópias mascaradas antes de carregar em staging.
...
4. Registrar em audit trail cada execução do processo de masking e cada exportação.

## Testes e QA

- Cobrir com testes unitários e E2E que verifiquem o mascaramento de campos sensíveis.
- Incluir casos de borda: nulos, variações de nomes de campo, listas aninhadas e streaming.

## Responsabilidades

- **Devs Backend**: implementar e manter a biblioteca de masking e scripts de staging.
- **Time de Segurança**: definir catálogo final de campos sensíveis e aprovar procedimentos.
- **PO / Compliance**: aprovar a política e o fluxo de promoção.

## Referências

- DECISAO-002 — aplicar caso a política exija mudanças arquiteturais.
- `scripts/mask_sqlite.py` — utilitário no repositório.
...
Origin: TASK-15 - Documentar políticas de masking
<!--
Padrão: Todo novo documento deve ter título sem pontuação final, linhas em branco após headings e estar em Português.
-->
# Politica de Masking (rascunho)

Origin: AG-005 - Implementação inicial de Masking

Resumo
---
Este documento descreve a política mínima para mascaramento (masking) de dados sensíveis (PII) em ambientes não-produtivos e nos endpoints de API que expõem informações de usuários ou clientes. É um rascunho técnico que deve ser complementado pelo time de Segurança/Governança.

Escopo
---
- Aplicável a: staging, ambientes locais com dados reais, dumps/exports e endpoints administrativos.
- Não aplicar em ambientes de produção onde dados completos são necessários para operação; em produção, usar controles de acesso (RBAC) e logging com retenção/roteamento seguro.

Definições
---
- PII: Personally Identifiable Information — exemplos: nome, email, CPF/SSN, telefone, documentos.
- Masking: transformação irreversível/irreversível parcial que remove/oculta PII para reduzir risco de exposição.

Regras básicas (mínimo)
---
1. Endpoints públicos ou de staging devem retornar PII mascarado por padrão.
2. Campos sensíveis iniciais (heurística): `name`, `email`, `cpf`, `ssn`, `document`, `phone`, `telefone`.
3. Para exportações administrativas (CSV/JSON), somente usuários com role `admin` podem exportar dados completos; logs de exportação são auditados.
4. Scripts de preparação de staging (ex.: `scripts/mask_sqlite.py`) devem ser usados para gerar cópias mascaradas antes de carregar em staging.
