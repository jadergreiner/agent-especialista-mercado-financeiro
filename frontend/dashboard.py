...
def get_dashboard_data():
    try:
        response = requests.get("http://localhost:8002/api/v1/dashboard/summary", timeout=5)
        response.raise_for_status()
        return response.json()
    except requests.exceptions.RequestException as e:
        st.error(f"Erro ao conectar com a API: {e}")
        st.info("Certifique-se de que o backend está rodando: `uvicorn backend.api.dashboard:app --host 0.0.0.0 --port 8002 --reload`")
        return None
